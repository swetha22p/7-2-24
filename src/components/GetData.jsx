...
const deletedIdDB = await openDB('deletedIdDB', 1, { 
  upgrade(db) {
      db.createObjectStore('deletedIds', { keyPath: 'id' , autoIncrement: true});
  },
});
...
const MedicalRecordsTable = ({ medicalRecords, onAddClick, onSearch, setMedicalRecords }) => {
  ...
  const checkDeletedIds = async () => {
    try {
        // Open the deletedIdDB database and check if the 'deletedIds' object store has any records
        const deletedIdDB = await openDB('deletedIdDB', 1);
        const deletedIdTransaction = deletedIdDB.transaction('deletedIds', 'readonly');
        const deletedIdObjectStore = deletedIdTransaction.objectStore('deletedIds');

        const allRecords = await deletedIdObjectStore.getAll();
        setRecords(allRecords)

        if (allRecords.length > 0) {
            // Display red flags or perform some action to indicate that records are deleted
            allRecords.forEach(({ id }) => {
                const recordElement = document.getElementById(`record-${id}`);
                console.log('Current Record Element:', recordElement);
                if (recordElement) {
                    recordElement.innerHTML += ' 🚩'; // Add a red flag symbol to the existing content
                }
                console.log('Red flag displayed for record with id:', id);
            });

            // Clear the 'deletedIds' object store
            const clearTransaction = deletedIdDB.transaction('deletedIds', 'readwrite');
            const clearObjectStore = clearTransaction.objectStore('deletedIds');
            clearObjectStore.clear();
            await clearTransaction.done;

            console.log('Cleared the deletedIds object store');
        } else {
            console.log('No red flags needed, as there are no deleted records');
        }
    } catch (error) {
        console.error('Error checking and clearing deletedIds:', error);
    }
};
console.log(records);
// Example usage in your getData page
...
// Function to handle closing image dialog
  ...
  const syncDeletedRecords = async () => {
    ...
    try {
      ...
      for (const { data, file ,id , firstName, lastName, email, phoneNumber, dob, gender, height, weight, bmi, disease, timestamp } of deletedRecordsBeforeSync) {
        let formData = {};
        ...
        try {
          const existsInMongoDB = await axios.get(`http://localhost:5000/medicalForm/${formData.id}`);
  
          // Existing entry found, update it
          const syncResponse = await axios.put(`http://localhost:5000/medicalForm/${existsInMongoDB.data.id}`, {
            ...formData,
            id: id,
            fileUrl,
            updatedAt: new Date(),
  ...
  const handleDelete = async (id, timestamp) => {
    ...
    try {
        ...
        const _id = originalRecord.data ? originalRecord.data.id : originalRecord.id;
       if (new Date(timestamp) > new Date(lastSyncTimestamp)) {
        ...
        await deletedTransaction.done;
           ...
           const deletedIdDB = await openDB('deletedIdDB', 1);
          const deletedIdTransaction = deletedIdDB.transaction('deletedIds', 'readwrite');
            const deletedIdObjectStore = deletedIdTransaction.objectStore('deletedIds');
            await deletedIdObjectStore.add({ id , _id, timestamp });
            console.log('Recorded deleted id in deletedIdDB:', id);
            await deletedIdTransaction.done;
  ...
  const handleDelete1 = async (id) => {
    ...
    try {
      ...
      const db = await openDB('medicalFormDB', 1);
      const deleteTransaction = db.transaction('medicalForms', 'readwrite');
      const deleteObjectStore = deleteTransaction.objectStore('medicalForms');
  
      // Wait for the transaction to complete
      await deleteObjectStore.delete(id);
      console.log('Deleted record from IndexedDB with id:', id);
  
      // Update the state to remove the deleted record (if applicable)
        ...
        <Table style={{ tableLayout: 'auto' }}>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Deleted Data</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
  ...
  const handleImportToIndexedDB = async () => {
    try {
      ...
      if (medicalRecords.length > 0) {
        ...
        await store.clear();
        medicalRecords.forEach(async (data) => {
